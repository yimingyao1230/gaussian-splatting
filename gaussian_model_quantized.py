...
def generate_codebook(values, inverse_activation_fn=lambda x: x, num_clusters=256):
    '''shape = values.shape
    ...
    kmeans = KMeans(n_clusters=num_clusters, random_state=0).fit(values_np)

    centers = torch.tensor(kmeans.cluster_centers_, device='cuda', dtype=values.dtype)
    ids = torch.tensor(kmeans.predict(values_np), device='cuda', dtype=torch.uint8).view(values.shape)

    return Codebook(ids, inverse_activation_fn(centers))
...
class GaussianModel:
    ...
    def save_ply_original(self, path):
    ...
    def generate_k_mean_clusters(self, num_clusters=256):
        max_coeffs_num = (self.max_sh_degree + 1)**2 - 1
        codebook_dict = OrderedDict({})
        print("[1/7] Clustering: DC features (3D color)")
        codebook_dict["features_dc"] = generate_codebook(self._features_dc.detach()[:, 0],
                                                         num_clusters=num_clusters)
        for sh_degree in range(max_coeffs_num):
                print(f"[2/7] Clustering: SH coefficient {sh_degree+1}/{max_coeffs_num}")
                codebook_dict[f"features_rest_{sh_degree}"] = generate_codebook(
                    self._features_rest.detach()[:, sh_degree], num_clusters=num_clusters)
        print("[3/7] Clustering: Opacity")
        codebook_dict["opacity"] = generate_codebook(self.get_opacity.detach(),
                                                     self.inverse_opacity_activation, num_clusters=num_clusters)
        print("[4/7] Clustering: Scaling (3D)")
        codebook_dict["scaling"] = generate_codebook(self.get_scaling.detach(), self.scaling_inverse_activation, num_clusters=num_clusters)

        print("[5/7] Clustering: Rotation (real part)")
        codebook_dict["rotation_re"] = generate_codebook(self.get_rotation.detach()[:, 0:1], num_clusters=num_clusters)

        print("[6/7] Clustering: Rotation (imaginary part)")
        codebook_dict["rotation_im"] = generate_codebook(self.get_rotation.detach()[:, 1:], num_clusters=num_clusters)

        print("[7/7] Clustering complete!")
